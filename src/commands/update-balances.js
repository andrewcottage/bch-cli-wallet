...
class UpdateBalances extends Command {
  ...
  // Retrieves data for every address generated by the address.
  async getAllAddressData(walletInfo) {
    try {
      ...
      const lastAddressDetected = false // Flag to signal when last address found.

      while (!lastAddressDetected) {
        // Get a 20-address batch of data.
        const thisDataBatch = await getAddressData(walletInfo, currentIndex, 20)

        // Add data to the array.
        addressData = addressData.concat(thisDataBatch)

        // Check if data has no balance. no balance == last address.
        lastAddressDetected = this.detectBalance(thisDataBatch)
      }
    } catch (err) {
